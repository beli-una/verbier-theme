function initHotelDetails() {
    ...
    } else $(".payment_methods_header").hide();
    $(".website_link").attr("href", $(".web").data("web_url")), hide_empty_data(".booking", "booking_url", ".hotels_book_button", "href"), 
    $(".uw_ts_rates tr").length < 2 && $(".uw_ts_rates").hide(), $(".uw_ts_item_details h3").length < 2 && $(".item_details_details").hide(), 
    $(".uw_ts_rates tr").length < 2 && 0 == $(".payment_type img").length && $(".item_details_rates").hide();
    var custome_filter = {
        filter: {
            rating: $(".search_params").data("rating"),
            exclude_item: $(".search_params").data("id")
        }
    };
    vdm = new VerbierDataManager(), vdm.getData("hotels.tmpl.js", ".uw_tourism_event_list", custome_filter, basicDataHandler);
}